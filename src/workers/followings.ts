...
export default class FollowingsWorker extends BaseWorker<IData> {

	public getDataType(): EData {
		return EData.FOLLOWINGS;
	}
	...
	protected async _perform(client: TwitterApiReadOnly, data: IData): Promise<void> {
		...
		this._processBulk(userData.id, username, followings.data);
		await Downloader.handlePaginationToken(
			this.getDataType(),
			userData,
			followings.meta.next_token,
			followings.meta.result_count,
		);
		if (!followings.done) {
